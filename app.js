        ...
        function updatePills(qObj) {
          ...
          if (qObj) {
            ...
            diffPill.textContent = `Difficulty: ${difficultyLabel(
              d
            )} • ${d} pt${d === 1 ? "" : "s"}`;
          } else {
        ...
        function uniqueCategories() {
          const cats = new Set(QUESTION_BANK.map(getCategory).filter(Boolean));
        ...
        function getAvailableQuestions(category) {
          return QUESTION_BANK.filter((q) => {
            const matchesCat =
              category === "ALL" || getCategory(q) === category;
  ...
  return picked;
        ...
        function resetRound(advanceRound = true) {
          ...
          if (advanceRound) {
            currentRoundIndex =
              (currentRoundIndex + 1) % GAME_CONFIG.rounds.length;
          }
          updatePills(null);
        }
      ...
      `;
        ...
        function updateWinners(sparkle = false) {
          ...
          const withTies = sorted.filter(
            (p) => p.score >= cutoffScore && p.score > 0
          );

          if (!withTies.length) {
            winnersArea.textContent =
              "No points yet—everyone’s still perfect. 😄";
            return;
          }
